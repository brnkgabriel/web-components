...
class MTextAreaOutlined extends HTMLElement {
  constructor() {
    super()
  ...
  html() {
    ...
    var tarea = new Tag(prop['tarea']).get()
    var span  = new Tag(prop['span']).get()

    tarea.addEventListener('keyup', () => { self.value = tarea.value })

    Tag.appendMany2One([tarea, span], label)
    return label
  }
...
customElements.define('m-tarea-outlined', MTextAreaOutlined)